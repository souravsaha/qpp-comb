...
def compute_regression():
    ...
    parser.add_argument("--dataset", type = str, choices = ["trec678rb", "trec678", "trecdl19", "trecdl",
    ...
    #for seed in range(1): 
        ...
        #kf = KFold(n_splits = 2, shuffle = True, random_state = 7 + 2652124)
        ...
        for train_index, test_index in kf.split(ap_real_pred):
            ...
            #regr_model = Lars(n_nonzero_coefs = 10)
            #regr_model =  LarsCV(cv =5)
            #regr_model = Lars()
            #regr_model = LassoCV(cv = 5, random_state = 0)
            # Trying Support Vector Regression
            #regr_model = make_pipeline(StandardScaler(), SVR(C=0.05, epsilon=0.05))
            ...
            if ols_type == "ols":
                regr_model = LinearRegression()
            elif ols_type == "lars-cv":
                regr_model =  LarsCV(cv =5)
            elif ols_type == "enet":
                regr_model = ElasticNetCV(cv = 5, random_state = 0, max_iter = 50000)
            elif ols_type == "lasso-cv":
                regr_model = LassoCV(cv = 5, random_state = 0, max_iter = 50000)
            elif ols_type == "ridge":
                regr_model = RidgeCV(cv = 5)
