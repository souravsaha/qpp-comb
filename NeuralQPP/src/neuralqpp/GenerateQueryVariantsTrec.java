...
public class GenerateQueryVariantsTrec {
    ...
    public GenerateQueryVariantsTrec(Properties prop) throws IOException, Exception {
...
//        fieldToSearch = prop.getProperty("fieldToSearch", FIELD_FULL_BOW);
        fieldToSearch = prop.getProperty("fieldToSearch", FIELD_BOW); // MsMarco
...
//            resFileWriter.close();
            ...
            switch(varGen.toLowerCase()) {
                case "rlm":
                    ...
                    resBuffer = new StringBuffer();
                    queryVariantList = qvrlm.createVariantsRLMTrec(topRetDocs, query, luceneQuery, varLen);
//                    resBuffer.append(query.qid).append(" ::: ").append(query.qtitle).append("\n");
                    for (String variant : queryVariantList) {
                        resBuffer.append(varId + 1).append("\t").
