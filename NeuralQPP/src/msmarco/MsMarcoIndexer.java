...
public class MsMarcoIndexer {
    ...
    void indexCollection() throws Exception {
        IndexWriter writer;
        IndexWriterConfig iwcfg = new IndexWriterConfig(constructAnalyzer());
        iwcfg.setOpenMode(IndexWriterConfig.OpenMode.CREATE);
        writer = new IndexWriter(FSDirectory.open(new File(Constants.MSMARCO_INDEX).toPath()), iwcfg);
        indexCollection(new FileInputStream(new File(Constants.MSMARCO_COLL)), writer);
        writer.close();
    }

    static String normalizeNumbers(String content) {
    ...
    void indexCollection(InputStream is, IndexWriter writer) throws Exception {
        ...
        while ((line = br.readLine())!= null) {
            ...
            if (parts.length >= 1) {
                ...
                String content = parts[1]; // normalizeNumbers(parts[1]);
