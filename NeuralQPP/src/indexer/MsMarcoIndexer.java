...
public class MsMarcoIndexer {
    ...
    public MsMarcoIndexer(String propFile) throws Exception {
        prop = new Properties();
...
//        saxParser.parse(is, handler);        
    ...
    public static void main(String[] args) {
        if (args.length == 0) {
            System.err.println("Usage: java MsMarcoIndexer <prop-file>");
            return;
        }

        try {
            MsMarcoIndexer indexer = new MsMarcoIndexer(args[0]);
            indexer.processAll();
        }
        catch (Exception ex) {
            ex.printStackTrace();
