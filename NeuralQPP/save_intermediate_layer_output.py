...
DATADIR = './exp_hist_with_idf/input_10/'   # (1)
DATADIR_idf = './exp_hist_with_idf/input_10/'
...
allPairs_test = PairedInstanceIds(DATADIR + 'test_input/qid_ap.pairs.gt')    # (4)
...
def build_siamese(input_shape):
    input_a = Input(shape=input_shape, dtype='float32', name='input_1')
    input_b = Input(shape=input_shape, dtype='float32', name='input_2')
    ...
    matrix_encoder.add(Dense(500, activation='relu'))
    ...
    predictions = Dense(1, activation='sigmoid', name='dense')(merged_vector)

    siamese_net = Model([input_a, input_b], outputs=predictions, name='out')
    return siamese_net
...
save_layer = open(DATADIR_idf + 'submodel.train','a')

for qidPair in allPairsList_train:
    training_generator = PairCmpDataGeneratorTrain(qidPair, dataFolder=DATADIR_idf+'train_input/')
